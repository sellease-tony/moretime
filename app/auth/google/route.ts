...
export async function GET(request:Request){
  ...
  if(!supabaseConfigured())return NextResponse.redirect(new URL('/?auth_error=configuration',origin));
  ...
  if(error||!data.url)return NextResponse.redirect(new URL('/?auth_error=signin',origin));
