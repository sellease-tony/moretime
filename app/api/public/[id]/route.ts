...
export async function GET(request:Request,context:Context){
  ...
  try{
    ...
    const {state}=await loadOwner(target.owner_id),event=state.events.find(e=>e.id===target.event_id&&e.active);if(!event)return json({error:'현재 예약을 받고 있지 않습니다.'},404);
    const day=new URL(request.url).searchParams.get('day');let slots:string[]=[];
    if(day){if(!validDay(day))return json({error:'오늘부터 90일 이내의 날짜를 선택해 주세요.'},400);slots=(await slotsFor(target.owner_id,target.event_id,day)).slots}
    const user=await currentUser();
    return json({event,slots,mode:notificationMode(),user:user?{name:user.user_metadata.full_name||user.email,email:user.email}:null});
...
export async function POST(request:Request,context:Context){
  ...
  const user=await currentUser();if(!user?.email)return json({error:'Google 로그인 후 예약할 수 있습니다.'},401);
  try{
    const target=await link((await context.params).id);if(!target)return json({error:'예약 페이지를 찾을 수 없습니다.'},404);
    ...
    // Fresh Google call immediately before the database transaction; never trust client slots.
    ...
    const parsed=bookingSchema.safeParse({id:body.id,eventId:event.id,title:event.title,duration:event.duration,day:body.day,time:body.time,name:body.name,email:user.email,phone:body.phone||'',channels:[...new Set(['email',...(Array.isArray(body.channels)?body.channels:[])])],notificationConsent:body.notificationConsent===true});
    if(!parsed.success)return json({error:parsed.error.issues[0].message},400);
    const booking=parsed.data;
    const db=adminClient();
    // Limit per verified Google account, avoiding anonymous arbitrary-recipient mail.
    const {count,error:limitError}=await db.from('moa_bookings').select('id',{head:true,count:'exact'}).eq('payload->>email',user.email).gte('created_at',new Date(Date.now()-86400000).toISOString());
