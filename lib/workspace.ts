...
export function validateNewBooking(b:Booking,state:typeof defaultWorkspace,now=new Date()){
  ...
  if(date<=now||!state.hours[weekday]||minutes(b.time)<minutes(state.range[0])||minutes(b.time)+b.duration>minutes(state.range[1]))throw Error('예약할 수 없는 시간입니다.');
}
