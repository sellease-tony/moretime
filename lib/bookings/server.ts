...
export async function slotsFor(owner:string,eventId:string,day:string){
  ...
  const start=new Date(day+'T00:00:00+09:00'),end=new Date(start.getTime()+86400000);
  const busy=await ownerBusy(owner,start.toISOString(),end.toISOString());
  return {slots:availableSlots(day,event.duration,data.state.hours,data.state.range,[...busy,...data.busy]),event,data};
}
